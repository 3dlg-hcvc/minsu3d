...
from lib.utils.projection import ProjectionHelper
...
def get_scene_data(cfg, scene_list, split):
    ...
    for scene_id in tqdm(scene_list):
        ...
        scene_data[scene_id] = data["mesh"][:, :3] # NOTE must use the original mesh data!!!
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cfg', help='scannet configuration YAML file', default='../../conf/path.yaml')
        ...
        for split in ["train", "val", "test"]:
            ...
            for scene_id in scene_list:
                ...
                for i, frame_id in enumerate(frame_list):
                    scene_images[i] = load_image(os.path.join(cfg.SCANNETV2_PATH.frames, scene_id, "color", "{}.jpg".format(frame_id)), [328, 256])
                    scene_depths[i] = load_depth(os.path.join(cfg.SCANNETV2_PATH.frames, scene_id, "depth", "{}.png".format(frame_id)), [41, 32])
                    scene_poses[i] = load_pose(os.path.join(cfg.SCANNETV2_PATH.frames, scene_id, "pose", "{}.txt".format(frame_id)))
                ...
                for i, projection in enumerate(projections):
                    ...
                    projection_3d = projection[1]
                    projection_2d = projection[2]
                    feat = to_tensor(np.load(os.path.join(cfg.SCANNETV2_PATH.enet_features, scene_id, "{}.npy".format(frame_id))))
