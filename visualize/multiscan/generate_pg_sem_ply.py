...
def generate_pred_inst_ply():
    ...
    data_dir = "/localhome/yza440/Research/HAIS/data/multiscan_inst/train"
    pred_dir = f'/localhome/yza440/Research/HAIS/exp/multiscan_inst/hais/hais_run1_multiscan_inst/result' # TODO
    output_dir = f'/localhome/yza440/hais_train_ply' # TODO
    os.makedirs(output_dir, exist_ok=True)
    scene_ids_file = sorted(glob.glob(os.path.join(data_dir, "*.pth")))
    scene_ids_file = [name.split("/")[-1][:-4] for name in scene_ids_file]
    # scene_ids = [scene_id.rstrip() for scene_id in open(scene_ids_file)]
    scene_ids = ['20210825T101506-0700_BEC82A3A-F9BC-4A6F-9321-5CE73A848E62']
    global color_cache
    ...
    for scene_id in scene_ids:
        ...
        with open(pred_inst_file, "r") as f:
            for i, line in enumerate(f):
                ...
                mask_idx = np.loadtxt(obj_pred_file, dtype=np.uint32)
        ...
        random_colors = np.zeros((100, 3))
        for i in range(100):
            cmap = matplotlib.colors.ListedColormap(np.random.rand(256, 3))
            random_colors[i] = np.asarray(cmap(i))[:3]
        ...
        for i, inst_id in enumerate(unique_inst_ids):
            ...
            colors[inst_vert_idx, :] = random_colors[i] * 255
            # colors[inst_vert_idx, :] = LABEL_COLORS[i]
        original_mesh_file = os.path.join('/localhome/yza440/Research/multiscan_dataset', scene_id, scene_id + "_clean.ply")
        plydata = PlyData.read(original_mesh_file)
        write_ply_rgb(points, colors, rgb_inst_ply, normals, plydata)
...
def generate_pred_sem_ply():
    split = "val"
    output_dir = f'/local-scratch/localhome/yza440/pg/vis'
    # pred_dir = f'/localhome/yza440/Research/inst_pred_to_part/SSTNet/semantic' # TODO
    pred_dir = "/localhome/yza440/Research/pointgroup-minkowski_old/log/multiscan/pointgroup/test/110000_inst_latest/splited_pred/val/semantic"
    os.makedirs(output_dir, exist_ok=True)
