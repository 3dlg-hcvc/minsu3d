...
class BaseSeg(nn.Module):
    def __init__(self,
        ...
        if cls_args1 is not None:
            if hasattr(self.decoder, 'out_channels'):
                in_channels = self.decoder.out_channels
            elif hasattr(self.encoder, 'out_channels'):
                in_channels = self.encoder.out_channels
            else:
                in_channels = cls_args1.get('in_channels', None)
            cls_args1.in_channels = in_channels
            self.head1 = build_model_from_cfg(cls_args1)
        else:
            self.head1 = None
        if cls_args2 is not None:
            if hasattr(self.decoder, 'out_channels'):
                in_channels = self.decoder.out_channels
            elif hasattr(self.encoder, 'out_channels'):
                in_channels = self.encoder.out_channels
            else:
                in_channels = cls_args2.get('in_channels', None)
            cls_args2.in_channels = in_channels
            self.head2 = build_model_from_cfg(cls_args2)
        else:
            self.head2 = None

    def forward(self, data, head2_flag=True):
        p, f = self.encoder.forward_seg_feat(data)
        if self.decoder is not None:
            f = self.decoder(p, f).squeeze(-1)
        
        # Assumes that if head2 is defined, then head1 is also defined
        if self.head2 is not None and head2_flag:
            f1 = self.head1(f)
            f2 = self.head2(f)
            return f1, f2
        elif self.head1 is not None:
            f1 = self.head1(f)
            return f1, None
        else:
            return None, None
