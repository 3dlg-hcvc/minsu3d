    ...
    for b in range(bsize):
        ...
        if not config_dict['use_3d_nms']:
            ...
            for j in range(num_proposal_batch):
                boxes_2d_with_prob[j,0] = np.min(pred_corners_3d_upright_camera_batch[j,:,0])
            ...
            for j in range(num_proposal_batch):
                boxes_3d_with_prob[j,0] = np.min(pred_corners_3d_upright_camera_batch[j,:,0])
                boxes_3d_with_prob[j,1] = np.min(pred_corners_3d_upright_camera_batch[j,:,1])
                boxes_3d_with_prob[j,2] = np.min(pred_corners_3d_upright_camera_batch[j,:,2])
