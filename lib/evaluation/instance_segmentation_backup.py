            ...
            for ci, class_name in enumerate(CLASS_NAME):
                ...
                for m in matches:
                    ...
                    gt_instances = [gt for gt in gt_instances if
                                    gt['vert_count'] >= min_region_size and gt['med_dist'] <= distance_thresh and gt[
...
def evaluate_instance(cfg):
    ...
    pred_path = os.path.join(cfg.OUTPUT_PATH, cfg.general.dataset, cfg.general.model, cfg.evaluation.use_model, "test",
                             cfg.data.split, "instance")
